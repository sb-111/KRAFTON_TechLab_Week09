-- Template Lua Script
-- 이 파일은 새로운 스크립트를 생성할 때 복사
-- obj: 이 스크립트를 소유한 Actor 객체

function BeginPlay()
    print("[BeginPlay] Script started")

    -- 예시: Actor의 위치 출력
    local pos = obj:GetActorLocation()
    print(string.format("Initial Position: (%.2f, %.2f, %.2f)", pos.x, pos.y, pos.z))

    -- 테스트: Actor를 위로 100 유닛 이동
    obj:SetActorLocation(Vector(pos.x, pos.y, pos.z + 100))

    -- 테스트: Actor 크기를 2배로 키우기
    obj:SetActorScale(Vector(2, 2, 2))

    print("[Lua Test] Actor moved up 100 units and scaled to 2x")
end

function Tick(dt)
    -- 매 프레임 호출
    -- dt: Delta Time (초 단위)

    -- 예시: Actor를 위로 이동
    local velocity = Vector(0, 0, 10) -- 10 units/sec
    obj:AddActorWorldLocation(velocity * dt)
end

function EndPlay()
    print("[EndPlay] Script ended")
end

-- 커스텀 이벤트 (Overlap)
function OnOverlap(OtherActor)
    print("[OnOverlap] Overlapped with: " .. tostring(OtherActor:GetName()))
end
