...
{
	InitializeLineBatch();
	ShadowSystem = new FShadowSystem(InDevice);
	CSMSystem = new FCSM(InDevice);
	// 타일 라이트 컬러는 프레임 간 재사용 (컴파일/리소스 재생성 방지)
	TileLightCuller = new FTileLightCuller();
}
...
URenderer::~URenderer()
{
	...
	if (TileLightCuller)
	{
		delete TileLightCuller;
		TileLightCuller = nullptr;
	}
}
