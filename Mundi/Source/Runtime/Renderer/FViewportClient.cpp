...
void FViewportClient::Tick(float DeltaTime)
{
	...
	// Override camera with light’s perspective 처리
	static ULocalLightComponent* LastFrameSelectedLocalLightComp = nullptr;
	ULocalLightComponent* SelectedLocalLightComp = nullptr;
	bool bOverrideLightPerspective = ULocalLightComponent::DoesSelectedLightOverrideCameraTransform(&SelectedLocalLightComp);
	if (SelectedLocalLightComp && SelectedLocalLightComp != LastFrameSelectedLocalLightComp && bOverrideLightPerspective) // bOverrideLightPerspective가 처음으로 true가 되었거나, bOverrideLightPerspective가 true인 Light가 새로 선택되었을 때
	{
		if (USpotLightComponent* SpotLightComp = Cast<USpotLightComponent>(LastFrameSelectedLocalLightComp))
		{
			SpotLightComp->SetVisibleDirectionGizmo(true);
		}
		if (USpotLightComponent* SpotLightComp = Cast<USpotLightComponent>(SelectedLocalLightComp))
		{
			SpotLightComp->SetVisibleDirectionGizmo(false);
		}
		FVector LightPosition = SelectedLocalLightComp->GetWorldLocation();
		FQuat LightRotation = SelectedLocalLightComp->GetWorldRotation();
		Camera->SetActorLocation(LightPosition);
		Camera->SetActorRotation(LightRotation);
		LastFrameSelectedLocalLightComp = SelectedLocalLightComp;
	}
	else if (SelectedLocalLightComp && LastFrameSelectedLocalLightComp == SelectedLocalLightComp && bOverrideLightPerspective) // 원래 선택되어있었을 때
	{
		FVector CameraPosition = Camera->GetActorLocation();
		FQuat CameraRotation = Camera->GetActorRotation();
		SelectedLocalLightComp->SetWorldLocation(CameraPosition);
		SelectedLocalLightComp->SetWorldRotation(CameraRotation);
	}
	else // 선택된 light의 bOverrideLightPerspective가 false일 때 or 선택된 컴포넌트가 LocalLightComponent가 아닐 때
	{
		if (USpotLightComponent* SpotLightComp = Cast<USpotLightComponent>(LastFrameSelectedLocalLightComp))
		{
			SpotLightComp->SetVisibleDirectionGizmo(true);
		}
		LastFrameSelectedLocalLightComp = nullptr;
	}
...
void FViewportClient::MouseMove(FViewport* Viewport, int32 X, int32 Y)
{
	...
	if (!bIsMouseButtonDown &&
		//(!World->GetGizmoActor() || !World->GetGizmoActor()->GetbIsHovering()) && // 컴포넌트와 카메라의 위치가 완전히 일치하면, 마우스가 어떤 위치에 있어도 기즈모 호버링 상태가 되어버려서 주석 처리
		bIsMouseRightButtonDown) // 마우스 오른쪽 버튼이 눌려있을 때
	{
