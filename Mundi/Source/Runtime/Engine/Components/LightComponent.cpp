...
BEGIN_PROPERTIES(ULightComponent)
    ...
    ADD_PROPERTY_RANGE(float, ShadowSharpen, "Shadow", 0.0f, 1.0f, true,
        "그림자 선명도 (0=부드러움, 1=선명)")
	ADD_PROPERTY_RANGE(float, ShadowBias, "Shadow", 0.0f, 1.0f, true, "그림자 바이어스 값")
	ADD_PROPERTY_RANGE(float, ShadowSlopeBias, "Shadow", 0.0f, 1.0f, true, "그림자 경사 바이어스 값")
	ADD_PROPERTY_RANGE(float, MaxSlopeDepthBias, "Shadow", 0.0f, 1.0f, true, "최대 경사 깊이 바이어스 값")
END_PROPERTIES()
