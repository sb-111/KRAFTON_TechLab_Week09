...
public:
	...
	void SetOverrideLightPerspective(bool bInOverride) { bOverrideLightPerspective = bInOverride; }
	bool IsOverridingLightPerspective() const { return bOverrideLightPerspective; }
	...
	static bool DoesSelectedLightOverrideCameraTransform(ULocalLightComponent** OutSelectedLocalLightComp);
...
protected:
	...
	bool bOverrideLightPerspective = false; // 해당 라이트가 선택됐을 경우, 카메라 Transform을 선택된 라이트의 시점으로 강제 설정
};
