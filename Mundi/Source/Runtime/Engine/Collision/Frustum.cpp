...
bool Intersects(const FPlane& P, const FVector4& Center, const float Radius)
{
    const float Distance = Dot3(P.Normal, Center) - P.Distance;
    return Distance + Radius >= 0.0f;
}

bool IsBoundingSphereIntersects(const FFrustum& Frustum, const FBoundingSphere& InBound)
{
    const FVector4& Center =  FVector4::FromPoint(InBound.Center);
    const float Radius = InBound.Radius;
    return Intersects(Frustum.LeftFace, Center, Radius) &&
    Intersects(Frustum.RightFace, Center, Radius) &&
    Intersects(Frustum.TopFace, Center, Radius) &&
    Intersects(Frustum.BottomFace, Center, Radius) &&
    Intersects(Frustum.NearFace, Center, Radius) &&
    Intersects(Frustum.FarFace, Center, Radius);
}

bool IsAABBVisible(const FFrustum& Frustum, const FAABB& Bound)
{
    // AABB 중심/반길이
    const FVector Center3 = Bound.GetCenter();
    const FVector Extents3 = Bound.GetHalfExtent();  // 항상 양수
