...
void UConsoleWidget::ExecCommand(const char* command_line)
{
	...
	{
        // Shadow filter command: SHADOW_FILTER <NONE|PCF|VSM>
        if (Strnicmp(command_line, "SHADOW_FILTER", 13) == 0)
        {
            const char* arg = command_line + 13;
            while (*arg == ' ') ++arg;
            if (*arg == 0)
            {
                AddLog("Usage: SHADOW_FILTER NONE|PCF|VSM");
            }
            else
            {
                bool bHandled = false;
                ...
                if (World)
                {
                    ...
                    {
                        World->GetRenderSettings().SetShadowFilterMode(EShadowFilterMode::NONE);
                        AddLog("Shadow filter set to NONE");
                        bHandled = true;
                    }
                    if (Stricmp(arg, "PCF") == 0)
                    {
                        World->GetRenderSettings().SetShadowFilterMode(EShadowFilterMode::PCF);
                        AddLog("Shadow filter set to PCF");
                        bHandled = true;
                    }
                    else if (Stricmp(arg, "VSM") == 0)
                    {
                        World->GetRenderSettings().SetShadowFilterMode(EShadowFilterMode::VSM);
                ...
                if (!bHandled)
                {
                    AddLog("Unknown SHADOW_FILTER argument. Use NONE, PCF or VSM.");
                }
            }
        }
		else
