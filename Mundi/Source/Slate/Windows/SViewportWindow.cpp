...
void SViewportWindow::RenderToolbar()
{
	...
	if (ImGui::Begin(WindowId, nullptr, WindowFlags))
	{
		...
		// 모든 light의 섀도우 값 초기화 버튼
		if (ImGui::Button("Rest All Light's Shadow Value"))
		{
			GWorld->GetLightManager()->SetAllLightShadowInfoToDefault();
		}
		ImGui::SameLine();
...
void SViewportWindow::LoadToolbarIcons(ID3D11Device* Device)
{
	...
	IconShadows = NewObject<UTexture>();
	IconShadows->Load(GDataDir + "/Icon/Viewport_Shadows.png", Device);
...
void SViewportWindow::RenderShowFlagDropdownMenu()
{
	...
	if (ImGui::BeginPopup("ShowFlagPopup", ImGuiWindowFlags_NoMove))
	{
		...
		if (ImGui::BeginMenu("  뷰포트 통계"))
		{
            ...
            if (ImGui::IsItemHovered())
            {
                ImGui::SetTooltip("라이트 메모리/개수 통계를 표시합니다.");
            }

			/*bool bShadowStats = UStatsOverlayD2D::Get().IsShadowInfoVisible();
			if (ImGui::Checkbox(" Shadow", &bShadowStats))
			{
				UStatsOverlayD2D::Get().ToggleShadowInfo();
			}
			if (ImGui::IsItemHovered())
			{
				ImGui::SetTooltip("그림자 관련 통계를 표시합니다.");
			}*/
		...
		ImGui::SameLine();
		if (IconShadows && IconShadows->GetShaderResourceView())
		{
			ImGui::Image((void*)IconShadows->GetShaderResourceView(), IconSize);
			ImGui::SameLine(0, 4);
		}
		ImGui::Text(" 그림자");
		if (ImGui::IsItemHovered())
        ...
        // 그림자 필터 (Shadow Filter) - 섹션 하단에 배치
        if (ImGui::BeginMenu("그림자 "))
        {
            ImGui::TextColored(ImVec4(0.6f, 0.6f, 0.6f, 1.0f), "그림자 모드");
            ImGui::Separator();

			EDirectionalShadowMode dirMode = RenderSettings.GetDirectionaliShadowMode();
			bool dDefault = (dirMode == EDirectionalShadowMode::GENERAL);
			bool bCSM    = (dirMode == EDirectionalShadowMode::CSM);

			if (ImGui::Checkbox(" DEFAULT", &dDefault))
			{
				if (dDefault) RenderSettings.SetDirectionaliShadowMode(EDirectionalShadowMode::GENERAL);
			}
			if (ImGui::IsItemHovered())
				ImGui::SetTooltip("필터 적용 X");

			if (ImGui::Checkbox(" CSM", &bCSM))
			{
				if (bCSM) RenderSettings.SetDirectionaliShadowMode(EDirectionalShadowMode::CSM);
			}
			if (ImGui::IsItemHovered()) ImGui::SetTooltip("CSM 필터 적용");
            ImGui::EndMenu();
        }

        if (ImGui::IsItemHovered())
        {
            ImGui::SetTooltip("그림자 설정");
        }
        if (ImGui::BeginMenu("그림자 필터"))
        {
            ImGui::TextColored(ImVec4(0.6f, 0.6f, 0.6f, 1.0f), "그림자 필터 모드");
            ImGui::Separator();
        	
        	EShadowFilterMode filter = RenderSettings.GetShadowFilterMode();
        	bool bNone = (filter == EShadowFilterMode::NONE);
        	bool bPCF  = (filter == EShadowFilterMode::PCF);
        	bool bVSM  = (filter == EShadowFilterMode::VSM);

        	if (ImGui::Checkbox(" NONE", &bNone)) { if (bNone) RenderSettings.SetShadowFilterMode(EShadowFilterMode::NONE); }
        	if (ImGui::IsItemHovered())  ImGui::SetTooltip("필터 적용 X");

        	if (ImGui::Checkbox(" PCF", &bPCF))   { if (bPCF)  RenderSettings.SetShadowFilterMode(EShadowFilterMode::PCF); }
        	if (ImGui::IsItemHovered())  ImGui::SetTooltip("PCF 필터 적용");

        	if (ImGui::Checkbox(" VSM", &bVSM))   { if (bVSM)  RenderSettings.SetShadowFilterMode(EShadowFilterMode::VSM); }
        	if (ImGui::IsItemHovered()) ImGui::SetTooltip("VSM 필터 적용");

            ImGui::EndMenu();
        }
        if (ImGui::IsItemHovered())
        {
            ImGui::SetTooltip("그림자 필터 모드 선택");
        }

        // Directional Shadow Resolution (moved below filter)
        if (ImGui::BeginMenu("다이렉셔널 해상도"))
        {
            ImGui::TextColored(ImVec4(0.6f,0.6f,0.6f,1.0f), "Directional Shadow Resolution");
            ImGui::Separator();
            uint32 CurrentDirRes = RenderSettings.GetDirectionalShadowResolution();
            uint32 PendingDirRes = CurrentDirRes;
            bool d512 = (CurrentDirRes == 512);
            if (ImGui::Checkbox(" 512##Dir", &d512)) { if (d512) PendingDirRes = 512; }
            ImGui::SameLine();
            bool d1024 = (CurrentDirRes == 1024);
            if (ImGui::Checkbox(" 1024##Dir", &d1024)) { if (d1024) PendingDirRes = 1024; }
            ImGui::SameLine();
            bool d2048 = (CurrentDirRes == 2048);
            if (ImGui::Checkbox(" 2048##Dir", &d2048)) { if (d2048) PendingDirRes = 2048; }
            ImGui::SameLine();
            bool d4096 = (CurrentDirRes == 4096);
            if (ImGui::Checkbox(" 4096##Dir", &d4096)) { if (d4096) PendingDirRes = 4096; }

            if (PendingDirRes != CurrentDirRes)
            {
                RenderSettings.SetDirectionalShadowResolution(PendingDirRes);
                if (URenderer* Renderer = URenderManager::GetInstance().GetRenderer())
                {
                    if (auto* RHIDevice = Renderer->GetRHIDevice())
                    {
                        Renderer->GetShadowSystem()->SetDirectionalShadowTextureResolution(RHIDevice, PendingDirRes);
                        Renderer->GetCSMSystem()->SetShadowSize(PendingDirRes);
                        Renderer->GetCSMSystem()->RecreateResources(RHIDevice);
                    }
                }
            }
            ImGui::EndMenu();
        ...
        // Spot resolution (menu style like Directional)
        if (ImGui::BeginMenu("스팟 해상도"))
        {
            ImGui::TextColored(ImVec4(0.6f,0.6f,0.6f,1.0f), "Spot Shadow Resolution");
            ImGui::Separator();
            uint32 CurrentResolution = RenderSettings.GetSpotShadowResolution();
            uint32 PendingResolution = CurrentResolution;
            bool r512 = (CurrentResolution == 512);
            if (ImGui::Checkbox(" 512##Spot", &r512)) { if (r512) PendingResolution = 512; }
            ImGui::SameLine();
            bool r1024 = (CurrentResolution == 1024);
            if (ImGui::Checkbox(" 1024##Spot", &r1024)) { if (r1024) PendingResolution = 1024; }
            ImGui::SameLine();
            bool r2048 = (CurrentResolution == 2048);
            if (ImGui::Checkbox(" 2048##Spot", &r2048)) { if (r2048) PendingResolution = 2048; }
            if (PendingResolution != CurrentResolution)
            {
                RenderSettings.SetSpotShadowResolution(PendingResolution);
                if (URenderer* Renderer = URenderManager::GetInstance().GetRenderer())
                {
                    if (auto* RHIDevice = Renderer->GetRHIDevice())
                    {
                        Renderer->GetShadowSystem()->SetSpotShadowTextureResolution(RHIDevice, PendingResolution);
                    }
                }
            }
            ImGui::EndMenu();
        ...
        // Point resolution (menu style like Directional)
        if (ImGui::BeginMenu("포인트 해상도"))
        {
            ImGui::TextColored(ImVec4(0.6f,0.6f,0.6f,1.0f), "Point Shadow Resolution");
            ImGui::Separator();
            uint32 CurrentPointResolution = RenderSettings.GetPointShadowResolution();
            uint32 PendingPointResolution = CurrentPointResolution;
            bool p512 = (CurrentPointResolution == 512);
            if (ImGui::Checkbox(" 512##Point", &p512)) { if (p512) PendingPointResolution = 512; }
            ImGui::SameLine();
            bool p1024 = (CurrentPointResolution == 1024);
            if (ImGui::Checkbox(" 1024##Point", &p1024)) { if (p1024) PendingPointResolution = 1024; }
            ImGui::SameLine();
            bool p2048 = (CurrentPointResolution == 2048);
            if (ImGui::Checkbox(" 2048##Point", &p2048)) { if (p2048) PendingPointResolution = 2048; }
            if (PendingPointResolution != CurrentPointResolution)
            {
                RenderSettings.SetPointShadowResolution(PendingPointResolution);
                if (URenderer* Renderer = URenderManager::GetInstance().GetRenderer())
                {
                    if (auto* RHIDevice = Renderer->GetRHIDevice())
                    {
                        Renderer->GetShadowSystem()->SetPointShadowTextureResolution(RHIDevice, PendingPointResolution);
                    }
                }
            }
            ImGui::EndMenu();
