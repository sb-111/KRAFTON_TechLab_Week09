...
void AGizmoActor::UpdateComponentVisibility()
{
	...
	// LocalLightComponent의 bOverrideLightPerspective가 true이면 기즈모 렌더링 비활성화
	bool bShouldHideGizmo = false;
	if (bHasSelection && SelectionManager)
	{
		bool bIsOverridingCameraWithLight = ULocalLightComponent::DoesSelectedLightOverrideCameraTransform(nullptr);
		bShouldHideGizmo = bIsOverridingCameraWithLight;
	}

	// Arrow Components (Translate 모드)
	bool bShowArrows = bHasSelection && (CurrentMode == EGizmoMode::Translate) && !bShouldHideGizmo;
	...
	bool bShowRotates = bHasSelection && (CurrentMode == EGizmoMode::Rotate) && !bShouldHideGizmo;
	...
	bool bShowScales = bHasSelection && (CurrentMode == EGizmoMode::Scale) && !bShouldHideGizmo;
