...
// --- 타일 기반 라이트 컬링 리소스 ---
// t8: 타일별 라이트 인덱스 Structured Buffer (t2는 SpotShadow에서 사용 중)
// 구조:  [TileIndex * MaxLightsPerTile] = LightCount
//        [TileIndex * MaxLightsPerTile + 1 ~ ...] = LightIndices (상위 16비트: 타입 0=Point,1=Spot / 하위 16비트: 인덱스)
StructuredBuffer<uint> g_TileLightIndices : register(t8);
...
// b10: Viewport 상수 버퍼 (UI 영역 보정용)
cbuffer ViewportConstants : register(b10)
{
    float4 ViewportRect;   // xy = TopLeft, zw = Size (pixels)
    float4 ScreenSize;     // xy = RT Size, zw = 1/Size
};

// b12: 클러스터 컬링(Z-슬라이스) 설정 상수 버퍼 (PS)
// 주의: 기존 TileCullingBuffer와 별도로 유지하여 런타임 구조체 호환성 보장
cbuffer ClusterCullingBuffer : register(b13)
{
    uint  ClusterCountZ;   // Z 슬라이스 개수 (>=1이면 클러스터 모드)
