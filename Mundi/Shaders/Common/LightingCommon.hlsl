...
// 클러스터/타일 인덱스 계산 (픽셀 위치 + 월드 위치)
// 항상 3D 클러스터 인덱스(X,Y,Z) 사용 (TileCulling on 시)
// Z는 View-space Z를 로그 분할로 매핑 (ClusterCountZ==1이면 2D와 동일)
// SV_POSITION은 픽셀 중심 좌표 (0.5, 0.5 offset)
uint CalculateTileIndex(float4 screenPos, float3 worldPos)
{
    // 뷰포트 로컬 좌표로 변환 (UI/툴바 등 오프셋 보정)
    float2 local = screenPos.xy - ViewportRect.xy;
    ...
    // 3D 클러스터: View-space Z를 로그 분할하여 Z-슬라이스 계산
    uint sliceCountZ = max(ClusterCountZ, 1u);
    // 월드 → 뷰 공간 Z (D3D: +Z forward)
    float viewZ = mul(float4(worldPos, 1.0f), ViewMatrix).z;
    viewZ = max(viewZ, 0.000001f);
    float logNear = log2(max(ClusterNearZ, 0.000001f));
    float logFar  = log2(max(ClusterFarZ, ClusterNearZ + 0.000001f));
    float t = saturate((log2(viewZ) - logNear) / max(logFar - logNear, 0.000001f));
    uint sliceZ = (uint)min(floor(t * sliceCountZ), (float)(sliceCountZ - 1u));

    return sliceZ * (TileCountX * TileCountY) + (tileY * TileCountX + tileX);
}
